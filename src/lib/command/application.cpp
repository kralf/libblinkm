...
BlinkM::Command::Application::DefaultKeyedArguments::DefaultKeyedArguments() {
  (*this)["help"] = Argument("Display usage and exit", "", false);
  (*this)["version"] = Argument("Display version and exit", "", false);
}

BlinkM::Command::Application::ArgumentIndexError::ArgumentIndexError(
    int index) :
  Exception("Invalid argument index: %d", index) {
}

BlinkM::Command::Application::ArgumentKeyError::ArgumentKeyError(const
    std::string& key) :
  Exception("Invalid argument key: %s", key.c_str()) {
}

BlinkM::Command::Application::ArgumentFormatError::ArgumentFormatError(const
    std::string& key, const std::string& format) :
  Exception("Argument format error: --%s %s", key.c_str(), format.c_str()) {
}

BlinkM::Command::Application::UnexpectedArgumentError::UnexpectedArgumentError(
    const std::string& argument) :
  Exception("Unexpected argument: %s", argument.c_str()) {
}

BlinkM::Command::Application::MissingArgumentError::MissingArgumentError(
    const std::string& format) :
  Exception("Missing argument: %s", format.c_str()) {
}

BlinkM::Command::Application::Application(const std::string& description) :
  description(description),
  keyedArguments(Singleton<DefaultKeyedArguments>::getInstance()) {
}

BlinkM::Command::Application::Application(const Application& src) :
...
BlinkM::Command::Application::~Application() {
}
...
const std::string& BlinkM::Command::Application::getDescription() const {
  return description;
}

const std::string& BlinkM::Command::Application::getExecutable() const {
  return executable;
}

BlinkM::Command::Argument& BlinkM::Command::Application::operator[](
    int index) {
...
const BlinkM::Command::Argument& BlinkM::Command::Application::operator[](
    int index) const {
  if ((index >= 0) && (index < arguments.size()))
    return arguments[index];
  else
...
BlinkM::Command::Application& BlinkM::Command::Application::operator=(const
    Application& src) {
  description = src.description;
  executable = src.executable;
...
void BlinkM::Command::Application::addArgument(const Argument& argument) {
  arguments.push_back(argument);
}

void BlinkM::Command::Application::addArgument(const std::string& key, const
    Argument& argument) {
...
bool BlinkM::Command::Application::parseArguments(size_t argc, char** argv) {
...
void BlinkM::Command::Application::writeHelp(std::ostream& stream) const {
...
void BlinkM::Command::Application::writeVersion(std::ostream& stream) const {
